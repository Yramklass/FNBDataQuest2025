...
CONFIG_LFM = {
    ...
    "num_threads": 1,          # Number of parallel threads to use. (Might be 1 on Windows by default)
    "random_state": 42,
    "top_k": 10
}
device = torch.device("cuda" if torch.cuda.is_available() else "cpu") # For NDCG calculation

# --- 1. Load Data (Simplified for LightFM context) ---
def load_data_for_lightfm(file_path="../data/fnb_dataset/raw/dq_recsys_challenge_2025(in).csv"):
    ...
    # Use original string IDs for LightFM's Dataset mapping
...
# --- 2. Prepare Data using LightFM Dataset ---
def prepare_lightfm_data(df_all_interactions, df_positive_interactions):
    ...
    # The format is an iterable of (user_id, item_id, optional_weight)
    ...
    def build_feature_iter(df_source, id_col_str, feature_cols_map):
        # df_source should have unique IDs and their raw string features
    ...
    if all_user_feature_names:
        user_features_matrix = dataset.build_user_features(
            ...
            normalize=False # Normalization can sometimes help
        )
...
# --- 3. Train/Test Split ---
# LightFM's random_train_test_split splits interactions, not users.
# For user-based split, you'd adapt your previous splitting logic.
# Here, we'll use the interaction-based split for simplicity with LightFM's eval tools.
...
def evaluate_lightfm_model(model, test_interactions, train_interactions,
    ...
    # These work on LightFM's internal integer mapped IDs
    # train_interactions is used to exclude already seen items from recommendations
    ...
    # For NDCG, we need to get scores for all items for test users
    # LightFM's model.predict takes arrays of user and item *internal integer IDs*
